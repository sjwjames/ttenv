...
def plot_tracking_rate(adfq_dirs, dqn_dirs, pfdqn_dirs, mc_dirs, mc_greedy_dirs, seed_cnt, file_dir, speed_limits,
    ...
    if display[1]:
        ax.errorbar(x, dqn_data[:, 0], yerr=dqn_data[:, 1], fmt='-o', color='g', capsize=5, label='Gaussian DQN')
    if display[2]:
        ax.errorbar(x, pfds_data[:, 0], yerr=pfds_data[:, 1], fmt='-o', color='b', capsize=5, label='DPBQN')
    if display[3]:
        ax.errorbar(x, mc_data[:, 0], yerr=mc_data[:, 1], fmt='-o', color='c', capsize=5, label='DPBQN-PADS')
    if display[4]:
...
def plot_distancefigures(adfq_dirs, dqn_dirs, pfdqn_dirs, mc_dirs, mc_greedy_dirs, seed_cnt, file_dir,
    ...
    ax.errorbar(x, mc_data[0], color='c', label='Q-AIA')
...
def plot_test_results(adfq_dirs, dqn_dirs, pfdqn_dirs, mc_dirs, mc_greedy_dirs, seed_cnt, file_dir, speed_limits,
    ...
    if display[1]:
        axes.bar(x - 1. * width, dqn_data_means[0], width, yerr=dqn_data_stds[0],
                 capsize=5, label="Gaussian DQN", color="tab:green")
    if display[2]:
        axes.bar(x, pfds_data_means[0], width, yerr=pfds_data_stds[0],
                 capsize=5, label="DPBQN", color="tab:blue")
    if display[3]:
        axes.bar(x + 1. * width, mc_data_means[0], width, yerr=mc_data_stds[0],
                 capsize=5, label="DPBQN-PADS", color="tab:cyan")
    if display[4]:
    ...
    if display[0]:
        axes.bar(x - 1.5 * width, adfq_data_means[1], width, yerr=adfq_data_stds[1],
                 capsize=5, label="ADFQ", color="tab:red")
    if display[1]:
        axes.bar(x - 1. * width, dqn_data_means[1], width, yerr=dqn_data_stds[1],
                 capsize=5, label="Gaussian DQN", color="tab:green")
    if display[2]:
    ...
    if display[3]:
        axes.bar(x + 1. * width, mc_data_means[1], width, yerr=mc_data_stds[1],
                 capsize=5, label="DPBQN-PADS", color="tab:cyan")
    if display[4]:
    ...
    if len(speed_limits) > 1:
        ...
        axes.set_xlabel("Initial Distances")
    axes.set_ylabel("Runtime(s) per 100 steps")
    axes.set_title("Evaluation runtime")
    axes.legend()

    plt.savefig(file_dir + "runtime.pdf")


def calcaulte_tracking_rate(dists, d):
    indices = np.where(dists < d)[0]
    if len(indices) > 0:
        first_idx = indices[0]
        return len(indices) / len(dists[first_idx:])
    else:
        return 0.0
    ...
    for sl in speed_limits:
        for dist in distances:
            ...
            for i in range(seed_cnt):
                ...
                d1_rate_res += [calcaulte_tracking_rate(item, d) for item in d1]

                d2 = np.loadtxt(dqn_dirs[i] + str(sl) + "/" + str(dist) + "/" + 'distance_' + str(sl) + '.csv',
                                delimiter=',')
                d2_res.append(np.sum([1 if len(item[item < d]) > 0 else 0 for item in d2]) / 10.0)
                d2_rate_res += [calcaulte_tracking_rate(item, d) for item in d2]
                ...
                d3_rate_res += [calcaulte_tracking_rate(item, d) for item in d3]

                d4 = np.loadtxt(mc_dirs[i] + str(sl) + "/" + str(dist) + "/" + 'distance_' + str(sl) + '.csv',
                                delimiter=',')
                d4_res.append(np.sum([1 if len(item[item < d]) > 0 else 0 for item in d4]) / 10.0)
                d4_rate_res += [calcaulte_tracking_rate(item, d) for item in d4]
                ...
                d5_rate_res += [calcaulte_tracking_rate(item, d) for item in d5]
    ...
    if len(speed_limits) > 1:
        if display[0]:
            ax.errorbar(speed_limits, adfq_data[:, 0], yerr=adfq_data[:, 1], fmt='-o', color='r', capsize=5,
                        label='ADFQ')  # Use fmt for line and markers
        if display[1]:
            ax.errorbar(speed_limits, dqn_data[:, 0], yerr=dqn_data[:, 1], fmt='-o', color='g', capsize=5, label='Gaussian DQN')
        if display[2]:
            ax.errorbar(speed_limits, pfds_data[:, 0], yerr=pfds_data[:, 1], fmt='-o', color='b', capsize=5,
                        label='DPBQN')
        if display[3]:
            ax.errorbar(speed_limits, mc_data[:, 0], yerr=mc_data[:, 1], fmt='-o', color='c', capsize=5, label='DPBQN-PADS')
        if display[4]:
        ...
        if display[0]:
            ax.errorbar(speed_limits, adfq_data[:, 2], yerr=adfq_data[:, 3], fmt='-o', color='r', capsize=5,
                        label='ADFQ')  # Use fmt for line and markers
        if display[1]:
            ax.errorbar(speed_limits, dqn_data[:, 2], yerr=dqn_data[:, 3], fmt='-o', color='g', capsize=5, label='Gaussian DQN')
        if display[2]:
            ax.errorbar(speed_limits, pfds_data[:, 2], yerr=pfds_data[:, 3], fmt='-o', color='b', capsize=5,
                        label='DPBQN')
        if display[3]:
            ax.errorbar(speed_limits, mc_data[:, 2], yerr=mc_data[:, 3], fmt='-o', color='c', capsize=5, label='DPBQN-PADS')
        if display[4]:
        ...
        if display[0]:
            ax.errorbar(distances, adfq_data[:, 0], yerr=adfq_data[:, 1], fmt='-o', color='r', capsize=5,
                        label='ADFQ')  # Use fmt for line and markers
        if display[1]:
            ax.errorbar(distances, dqn_data[:, 0], yerr=dqn_data[:, 1], fmt='-o', color='g', capsize=5, label='Gaussian DQN')
        if display[2]:
            ax.errorbar(distances, pfds_data[:, 0], yerr=pfds_data[:, 1], fmt='-o', color='b', capsize=5,
                        label='DPBQN')
        if display[3]:
            ax.errorbar(distances, mc_data[:, 0], yerr=mc_data[:, 1], fmt='-o', color='c', capsize=5, label='DPBQN-PADS')
        if display[4]:
        ...
        if display[0]:
            ax.errorbar(distances, adfq_data[:, 2], yerr=adfq_data[:, 3], fmt='-o', color='r', capsize=5,
                        label='ADFQ')  # Use fmt for line and markers
        if display[1]:
            ax.errorbar(distances, dqn_data[:, 2], yerr=dqn_data[:, 3], fmt='-o', color='g', capsize=5, label='Gaussian DQN')
        if display[2]:
            ax.errorbar(distances, pfds_data[:, 2], yerr=pfds_data[:, 3], fmt='-o', color='b', capsize=5,
                        label='DPBQN')
        if display[3]:
            ax.errorbar(distances, mc_data[:, 2], yerr=mc_data[:, 3], fmt='-o', color='c', capsize=5, label='DPBQN-PADS')
        if display[4]:
...
def plot_particles(adfq_dirs, dqn_dirs, pfdqn_dirs, mc_dirs, mc_greedy_dirs, seed_cnt, file_dir, speed_limits,
                   distances, display):
    pfds_data = []
    adfq_data = []
    dqn_data = []
    ...
    for sl in speed_limits:
        for dist in distances:
            ...
            for i in range(seed_cnt):
                d1 = np.loadtxt(adfq_dirs[i] + str(sl) + "/" + str(dist) + "/" + 'distance_' + str(sl) + '.csv',
                                delimiter=',')
                d1_res.append(np.mean(np.sum(d1, axis=1)))

                d2 = np.loadtxt(dqn_dirs[i] + str(sl) + "/" + str(dist) + "/" + 'distance_' + str(sl) + '.csv',
                                delimiter=',')
                d2_res.append(np.mean(np.sum(d2, axis=1)))

                d3 = np.loadtxt(pfdqn_dirs[i] + str(sl) + "/" + str(dist) + "/" + 'distance_' + str(sl) + '.csv',
                                delimiter=',')
                d3_res.append(np.mean(np.sum(d3, axis=1)))

                d4 = np.loadtxt(mc_dirs[i] + str(sl) + "/" + str(dist) + "/" + 'distance_' + str(sl) + '.csv',
                                delimiter=',')
                d4_res.append(np.mean(np.sum(d4, axis=1)))

                d5 = np.loadtxt(mc_greedy_dirs[i] + str(sl) + "/" + str(dist) + "/" + 'distance_' + str(sl) + '.csv',
                                delimiter=',')
                d5_res.append(np.mean(np.sum(d5, axis=1)))
    ...
    ax.set_title('Particles Collected')
    ax.legend()  # Show the legend

    # Show the plot
    plt.savefig(file_dir + "particle_collection.pdf")
...
if __name__ == '__main__':
    ...
    seeds = [0, 1, 2, 3, 4]
    display = [False, True, True, True, False]
    if task != "non-markovian":
        adfq_dirs = [os.path.join(file_dir, "DPBQN/seed_0/test/seed_" + str(seed) + "/" + (
            "random_init/" if random_init else "")) for seed in seeds]
        dqn_dirs = [os.path.join(file_dir, "Gaussian_DQN/seed_0/test/seed_" + str(seed) + "/" + (
            "random_init/" if random_init else "")) for seed in seeds]
        pfdqn_dirs = [os.path.join(file_dir, "DPBQN/seed_0/test/seed_" + str(seed) + "/" + (
            "random_init/" if random_init else "")) for seed in seeds]
        pfdqn_pads_dirs = [os.path.join(file_dir, "TargetTracking-v1_1_0506085744/seed_0/test/seed_" + str(seed) + "/" + (
            "random_init/" if random_init else "")) for seed in seeds]
        mc_greedy_dirs = [
            os.path.join(file_dir, "TargetTracking-v1_1_0506085744/seed_0/test/seed_" + str(seed) + "/" + (
                "random_init/" if random_init else "")) for seed in seeds]
        plot_tracking_rate(adfq_dirs, dqn_dirs, pfdqn_dirs, pfdqn_pads_dirs, mc_greedy_dirs, len(seeds), file_dir, speed_limits,
                           distances, display)
        # plot_distance(adfq_dirs,dqn_dirs,pfdqn_dirs,len(seeds))
        plot_discovery_rate(adfq_dirs, dqn_dirs, pfdqn_dirs, pfdqn_pads_dirs, mc_greedy_dirs, len(seeds), file_dir,
                            speed_limits, distances, display)
        plot_test_results(adfq_dirs, dqn_dirs, pfdqn_dirs, pfdqn_pads_dirs, mc_greedy_dirs, len(seeds), file_dir, speed_limits,
                          distances, display)
        plot_distance_tracking_figures(adfq_dirs, dqn_dirs, pfdqn_dirs, pfdqn_pads_dirs, mc_greedy_dirs, len(seeds), file_dir,
                                       speed_limits, distances, display)
        # plot_distancefigures(adfq_dirs, dqn_dirs, pfdqn_dirs, mc_dirs, mc_greedy_dirs, len(seeds), file_dir,
        #                      speed_limits, distances)
        # if task in ["gas_obstacle", "gas_empty"]:
        #     plot_particles(adfq_dirs, dqn_dirs, pfdqn_dirs, pfdqn_pads_dirs, mc_greedy_dirs, len(seeds), file_dir,
        #                    speed_limits, distances, display)
