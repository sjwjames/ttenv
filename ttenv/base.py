...
class TargetTrackingBase(gym.Env):
    def __init__(self, num_targets=1, map_name='empty',
        ...
        self.observation_model = METADATA["observation_model"]
        ...
        if self.observation_model == LEAKAGE_OBS:
            self.leakage_model.plot_mu(self.target_init_pos, [self.MAP.mapmin[0], self.MAP.mapmax[0]],
                                       [self.MAP.mapmin[1], self.MAP.mapmax[1]], 1000)
        self.reset_num = 0
        self.init_pose = []
    ...
    def observe_and_update_belief(self):
        ...
        for i in range(self.num_targets):
            ...
            if observation[0]:  # if observed, update the target belief.
                ...
                if self.observation_model == LEAKAGE_OBS:
                    self.particles_observed.append(observation[1])
            else:
                if self.observation_model == LEAKAGE_OBS:
                    self.particles_observed.append(0)
        return observed
