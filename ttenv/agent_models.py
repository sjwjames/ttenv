...
class AgentDoubleInt2D_Nonlinear(AgentDoubleInt2D):
    ...
    def fx(self, x, dt):
        ...
        if self.collision_check(new_state[:2]):
            # collision repulsive force
            odom = [new_state[0], new_state[1], np.arctan2(new_state[3], new_state[2])]
            obs_pos = self.obs_check_func(odom)
            v = np.sqrt(np.sum(np.square(new_state[2:]))) + np.random.normal(0.0, 1.0)
            if obs_pos[1] >= 0:
                th = obs_pos[1] - (1 + np.random.random()) * np.pi / 2
            else:
                th = obs_pos[1] + (1 + np.random.random()) * np.pi / 2

            new_state = np.array([new_state[0], new_state[1], v * np.cos(th + odom[2]), v * np.sin(th + odom[2])])

        if self.obs_check_func is not None:
            odom = [new_state[0], new_state[1], np.arctan2(new_state[3], new_state[2])]
            obs_pos = self.obs_check_func(odom)
            speed = np.sqrt(np.sum(new_state ** 2))
            rot_ang = np.pi / 2 * (1. + 1. / (1. + np.exp(-(speed - 0.5 * METADATA['target_speed_limit']))))
            if obs_pos is not None:
                acc = max(0.0, speed * np.cos(obs_pos[1])) / max(METADATA['margin2wall'],
                                                                 obs_pos[0] - METADATA['target_speed_limit'] * dt * 2)
                th = obs_pos[1] - rot_ang if obs_pos[1] >= 0 else obs_pos[1] + rot_ang
                del_vx = acc * np.cos(th + odom[2]) * dt
                del_vy = acc * np.sin(th + odom[2]) * dt
            else:
                del_vx = 0.0
                del_vy = 0.0
            new_state[2] += del_vx
