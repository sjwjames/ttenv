...
class DynamicMap(GridMap):
    def __init__(self, map_dir_path, map_name, margin2wall=0.5):
        ...
        """
        # super().__init__(os.path.join(map_dir_path, map_name), margin2wall)
        map_config = yaml.load(open(os.path.join(map_dir_path, map_name + ".yaml"), "r"), Loader=yaml.BaseLoader)
        ...
        self.submap_coordinates = np.array(
            [[map_config['submaporigin'][2 * i], map_config['submaporigin'][2 * i + 1]] for i in
             range(4)]).astype(np.int16)
    ...
    def generate_map(self, chosen_idx=None, rot_angs=None, **kwargs):
        ...
        for (i, c_id) in enumerate(chosen_idx):
            ...
            rotated_obs_idx_global_0 = rotated_obs_idx_local[0] - int(rotated_obs.shape[0] / 2) + \
                                       int(self.submap_coordinates[i][0])
            rotated_obs_idx_global_1 = rotated_obs_idx_local[1] - int(rotated_obs.shape[1] / 2) + \
                                       int(self.submap_coordinates[i][1])
...
if __name__ == '__main__':
    ...
    d = DynamicMap(map_dir_path='.', map_name='dynamic_map')
    for _ in range(5):
        d.generate_map()
