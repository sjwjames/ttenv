...
from ttenv.dqn.utils import plot_gaussian_contours
...
def project_belief(agent, targets):
    res = np.array(agent)
    for target in targets:
        n = len(target)
        target_dim = len(target[0]) - 1
        gmm_cov = [np.eye(target_dim) * 0.01 for _ in range(n)]
...
class MCKNNModel:
    def __init__(self, target_dim, agent_dim, T, gamma=0.99, target_type="particle", n_neighbors=5, n_beliefs=10):
    ...
    def __call__(self, belief_states, agent_info, env_info, knn_state, greedy_flag=False, episode_step=None, **kwargs):
        ...
        q_vals = [0 for _ in action_map.values()]
        for i, action in enumerate(action_map.values()):
            ...
            for target_belief in target_beliefs:
                ...
                gmm_cov = [np.eye(state_dim) * 0.1 for _ in target_belief.weights]
                ...
                selected_zs = predictive_gmm.sample(self.n_beliefs)
                ...
                r_mean = np.mean(
                    [predictive_gmm.sg_entropy_ub() - next_b.sg_entropy_ub() for next_b
                     in
                     gmm_next_beliefs])
                if greedy_flag or episode_step + 1 == self.T:
                    v_future = 0.0
                else:

                    # assert len(self.values) == len(self.kd_tree.data), "incompatible data size"
                    # v_future = np.mean([self.knn_regressor.predict(np.concatenate(
                    #     (
                    #         next_agent_state,
                    #         np.concatenate((np.expand_dims(next_b[0], axis=1), next_b[1]), axis=1).flatten()),
                    #     axis=0)) for next_b in sample_next_beliefs])
                    gmm_mms = [next_b.compute_mms() for next_b in gmm_next_beliefs]
                    # v_future = np.mean([self.knn_regressor.predict(
                    #     np.concatenate((next_agent_state, next_b[0], np.array(next_b[1]).flatten()), axis=0)) for next_b
                    #     in gmm_mms])
                    states = [np.concatenate((next_agent_state, next_b[0], np.array(next_b[1]).flatten()), axis=0)
                              for
                              next_b
                              in gmm_mms]
                    # indices, distances = self.kd_tree.query_radius(states, 1.0, return_distance=True)
                    distances, indices = self.kd_trees[episode_step + 1].query(np.array(states), k=self.n_neighbors)
                    v_futures = []
                    idx = 0
                    for distance_item, idx_item in zip(distances, indices):
                        ...
                        v_futures.append(
                            np.average(self.values[episode_step + 1][idx_item], axis=0, weights=normalized_distances))
                        # v_futures.append(np.max(self.values[idx_item]))
                        # if sampled_ys[idx][0]:
                        #     gaussians = [(np.array(states[idx][self.agent_dim:self.agent_dim + self.target_dim]),
                        #                   np.reshape(states[idx][self.agent_dim + self.target_dim:],
                        #                              [self.target_dim, self.target_dim]))] +[(item[self.agent_dim:self.agent_dim + self.target_dim],
                        #                   np.reshape(item[self.agent_dim + self.target_dim:],
                        #                              [self.target_dim, self.target_dim])) for item in np.array(self.kd_tree.data)[idx_item]]
                        #     plot_gaussian_contours(gaussians)
                        idx += 1
                    ...
                    v_future = np.mean(v_futures) if len(v_futures) > 0 else 0.0

                q_vals[i] += self.gamma * (r_mean + v_future)
        return q_vals
...
def learn(env,
    ...
    for t in range(max_timesteps):
        ...
        q_values = act(obs, stochastic=True, update_eps=exploration[min(t, len(exploration) - 1)],
                       greedy_flag=(t < learning_starts), episode_step=episode_step)
        v_val = np.max(q_values)
            ...
            if num_episodes % eval_check == 0:
                ...
                for e_e in range(eval_episodes):
                    ...
                    for t_eval in range(int(eval_steps)):
                        ...
                        obs["env_info"] = env_info
                        obs["knn_state"] = knn_state
                        q_values = act(obs, stochastic=True, update_eps=0.0, greedy_flag=(t < learning_starts),
                                       episode_step=t_eval)
                        action = np.argmax(q_values)
                        # Execute action and observe next state
                        next_obs, reward, terminated, truncated, info = env.step(action)
